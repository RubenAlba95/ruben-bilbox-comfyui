...
function setup_tooltips(combo_name) {
    console.log("Starting tooltip setup for:", combo_name);

    const cname = combo_name.replace("_", " ");
    if (!window.bilbox_promptgeek_data || !(cname in window.bilbox_promptgeek_data)) {
        console.warn("No data found for:", cname);
        return;
    }

    // Check if tooltip container exists, create if not
    let tooltip = document.querySelector('.bx-tooltip');
    if (!tooltip) {
    ...
    const observer = new MutationObserver((mutations, obs) => {
        const submenus = document.querySelectorAll(".submenu:not([data-bx-has-tooltip])");
        
        if (submenus.length > 0) {
            applyTooltips(submenus, cname, tooltip);
        }
    });
    ...
    const existingSubmenus = document.querySelectorAll(".submenu:not([data-bx-has-tooltip])");
    if (existingSubmenus.length > 0) {
        applyTooltips(existingSubmenus, cname, tooltip);
    }
}

function applyTooltips(menus, category, tooltip) {
    const data = window.bilbox_promptgeek_data[category];
    let currentTooltipMenu = null;
    let resizeObserver = null;

    const positionTooltip = (menu, tooltipElement) => {
        ...
        if (tooltipRect.height > spaceBelow && spaceAbove > tooltipRect.height) {
            // Position above if there's more space
            top = menuRect.top - tooltipRect.height - 5;
        } else {
            // Stay below but prevent bottom overflow
            top = Math.min(menuRect.top, viewportHeight - tooltipRect.height - viewportPadding);
        }
        ...
        if (left + tooltipRect.width > viewportWidth) {
            left = Math.max(
                viewportPadding,
                menuRect.left - tooltipRect.width - 18
            );
        }
    ...
    const createMouseEnterHandler = (menu, match) => () => {
        if (currentTooltipMenu) return;

        tooltip.innerHTML = `
            <img src="${relPath}/PromptGeek/${match.images[0]}" />
            <p>${match.description}</p>
        `;
        tooltip.style.display = 'block';
        currentTooltipMenu = menu;

        const img = tooltip.querySelector('img');
        let imageLoaded = false;

        const handlePositioning = () => {
            // Cancel pending position updates if mouse leaves
            if (!currentTooltipMenu) return;
            
            positionTooltip(menu, tooltip);
            
            // Set up resize observer for dynamic content changes
            if (resizeObserver) resizeObserver.disconnect();
            resizeObserver = new ResizeObserver(() => positionTooltip(menu, tooltip));
    ...
    menus.forEach(menu => {
        ...
        if (match && !menu.hasAttribute('data-bx-has-tooltip')) {
            menu.setAttribute('data-bx-has-tooltip', 'true');
            
            const mouseEnterHandler = createMouseEnterHandler(menu, match);
            
            menu.addEventListener('mouseenter', mouseEnterHandler);
            menu.addEventListener('mouseleave', mouseLeaveHandler);
            menu.addEventListener('click', mouseLeaveHandler);
        }
    });
}
...
app.registerExtension({
	...
	nodeCreated(node, app) {
		...
		{
            setup_tooltips(node.widgets[i].name);
